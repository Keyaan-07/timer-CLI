...
def reset():
    flag = input(f"Do you want to start the Timer again? ({timer_hours}:{timer_minutes}:{timer_seconds}) Reply ('Yes' / 'No') : ").upper()
...
def timer():
    global timer_seconds, timer_hours, timer_minutes
    print(end_statement)
    for i in range(5):
        print()
    i = True
    while i == True:
        try:
            i = False
            timer_seconds = int(input("Enter Seconds between 0-59 for timer: "))
            if timer_seconds > 59:
                timer_seconds = 59
    ...
    while i == True:
        try:
            i = False
            timer_hours = int(input("Enter hours for timer: "))
            if timer_hours < 0:
                timer_hours=0
